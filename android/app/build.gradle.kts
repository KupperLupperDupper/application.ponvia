...
// Release signing. CI (and anyone releasing) writes android/key.properties + the keystore;
// when it's absent (normal local dev), release builds fall back to the debug key so
// `flutter run --release` still works without the secret material.
...
android {
    ...
    defaultConfig {
        ...
        applicationId = "io.github.kupperlupperdupper.ponvia"
    ...
    buildTypes {
        release {
            // Use the real upload key when key.properties is present (CI releases);
            // otherwise fall back to the debug key so local release runs still build.
